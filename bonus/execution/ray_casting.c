...
void checkhit(t_ray *ray, t_cube *cube)
{
    ...
    x = (int)(ray->wall_Hit_x / PIXEL_SIZE);
    y = (int)(ray->wall_Hit_y / PIXEL_SIZE);
...
void check_for_H(t_ray *ray, float x_intercept, float y_intercept, t_cube *cube)
{
   ...
   int hit_wall = 0;
    ...
    {
        if (!check_wall(cube, next_hor_x, next_hor_y)) //7AYDT HNA DIK LFUNCTION LIKATCHECKI BIHA (ray->ray_facing_up? -1 : 0)
        {
            // if (ray->ray_facing_up)
            //     next_hor_y--;
...
void check_for_V(t_ray *ray, float x_intercept, float y_intercept, t_cube *cube)
{
   ...
   int hit_wall = 0;
    ...
    {
        if (!check_wall(cube, next_ver_x, next_ver_y)) //7AYDT HNA DIK LFUNCTION LIKATCHECKI BIHA (ray->ray_facing_left? -1 : 0);
        {
